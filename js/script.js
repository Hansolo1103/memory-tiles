...
const buttonDisabled = (element, boolean) => {
...
normal.addEventListener("click", function (e) {
  ...
  function normalmode() {
    ...
    function checkNormal(currentLevel) {
      if (
        userClicked.some((r) => gamePattern.includes(r)) ||
    ...
    function nextSequence() {
      ...
      var interval = setInterval(function () {
        if (i < gamePattern.length) {
          buttonDisabled(normalgrid, true);
...
hackerpp.addEventListener("click", function (e) {
  ...
  function runhpp() {
    ...
    start.onclick = function () {
      score.innerHTML =
        "Player-" + player1 + " Set Pattern for Player-" + player2;
  ...
  function nextSequence() {
    levelLen = playerSetPattern.slice(
      0,
      playerClickedPattern.length + 1
    ).length;
  ...
  function checkAnswer(currentLevel) {
    if (playerSetPattern.length == playerClickedPattern.length) {
      ...
      playerSetPattern[i] === Number(playerClickedPattern[currentLevel])
    ) {
      if (
        playerSetPattern.slice(0, levelLen).length ==
        playerClickedPattern.length
      ) {
        if (level > 0) {
          score.innerHTML = "Score: " + level;
        }
...
const changeColorOnClick = (element) => {
  for (let i = 0; i < element.length; i++) {
    element[i].onclick = () => {
      element[i].style.backgroundColor = "#fff";
      setTimeout(() => {
        element[i].style.backgroundColor = "#000";
      }, 150);
    };
  }
};
changeColorOnClick(normalgrid);
changeColorOnClick(hackergrid);
changeColorOnClick(hackerppgrid);
